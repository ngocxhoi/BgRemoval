<template>
  <div class="flex items-center justify-between gap-4 py-3">
    <NuxtLink to="/" id="logoFooter">
      <svg
        viewBox="0 0 370.00000000000006 60.61805555555556"
        class="h-auto w-32 sm:w-36 md:w-40"
      >
    ...
    <p
      class="flex-1 border border-gray-400 pl-4 text-sm text-gray-500 hidden sm:block"
    >
      Copyright ©NgocxhoiDev 2022 - {{ new Date().getFullYear() }} | All rights
      reserved
    </p>
    <ClientOnly>
      <div class="flex items-center gap-2">
        <div
          class="group cursor-pointer size-10 rounded-full shadow-md hover:shadow-[#3b5998] dark:shadow-gray-400 dark:hover:shadow-[#3b5998] flex items-center justify-center hover:scale-105"
        >
          <Icon
            name="lucide:facebook"
            size="20"
            class="group-hover:text-[#3b5998]"
          />
        </div>
        <div
          class="group cursor-pointer size-10 rounded-full shadow-md hover:shadow-[#e1306c] dark:shadow-gray-400 dark:hover:shadow-[#e1306c] flex items-center justify-center hover:scale-105"
        >
          <Icon
            name="lucide:instagram"
            size="20"
            class="group-hover:text-[#e1306c]"
          />
        </div>
        <div
          class="group cursor-pointer size-10 rounded-full shadow-md hover:shadow-[#1da1f2] dark:shadow-gray-400 dark:hover:shadow-[#1da1f2] flex items-center justify-center hover:scale-105"
        >
          <Icon
            name="lucide:twitter"
            size="20"
            class="group-hover:text-[#1da1f2]"
          />
        </div>
      </div>
    </ClientOnly>
